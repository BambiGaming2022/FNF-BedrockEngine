...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'school': // Week 6 - Senpai, Roses
				...
				if (!ClientPrefs.lowQuality)
				{
					var treeLeaves:BGSprite = new BGSprite('weeb/petals', repositionShit, -40, 0.85, 0.85, ['PETALS ALL'], true, 12);
			...
			case 'schoolEvil': // Week 6 - Thorns
				...
				if (!ClientPrefs.lowQuality)
				{
					var bg:BGSprite = new BGSprite('weeb/animatedEvilSchool', posX, posY, 0.8, 0.9, ['background 2'], true, 12);
					bg.scale.set(6, 6);
					bg.antialiasing = false;
					add(bg);

					bgGhouls = new BGSprite('weeb/bgGhouls', -100, 190, 0.9, 0.9, ['BG freaks glitch instance'], false, 12);
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")" + rpcOpponentText, iconP2.getCharacter(), true, songLength);
		#end
		...
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song
					...
					+ storyDifficultyText
					+ ")"
					+ rpcOpponentText, iconP2.getCharacter(), true,
					songLength
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")" + rpcOpponentText, iconP2.getCharacter());
		}
		#end
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'Health Drain':
				...
				var val2:Int = Std.parseInt(value2);
				if (Math.isNaN(val1) || val1 < 0 || val1 > 100)
					val1 = 0;
				if (Math.isNaN(val2) || val2 > 1 || val2 < 0)
					val2 = 1;
		...
		if (ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			else
			{
				if (opponentChart && SONG.player2 == 'spirit' && SONG.song.toLowerCase() == 'thorns')
				{
					...
					camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
					FlxTween.tween(dad, {x: boyfriend.x - 200, alpha: 0}, 1.7, {
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		switch (daRating)
		{
			case "shit": // shit
				...
				score = 50;
				shits++;
				if (ClientPrefs.millisecondColor)
				{
					msText.color = red;
				}
			case "bad": // bad
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		callOnLuas((opponentChart ? 'goodNoteHitFix' : 'opponentNoteHitFix'), [
			notes.members.indexOf(note),
			Math.abs(note.noteData),
			note.noteType,
			note.isSustainNote
