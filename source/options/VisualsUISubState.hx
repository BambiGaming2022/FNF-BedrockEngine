...
class VisualsUISubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		#if !mobile
		var option:Option = new Option('FPS & Memory Counter', 'If unchecked, hides FPS & Memory Counters.', 'showFPS', 'bool', true);
		addOption(option);
		option.onChange = onChangeFPSCounter;
		#end

		var option:Option = new Option('Hide Combo Sprite', "If checked, disables the Combo Sprite.", 'comboSpr', 'bool', false);
		addOption(option);

		var option:Option = new Option('Hide HUD', 'If checked, hides most HUD elements.', 'hideHud', 'bool', false);
		addOption(option);

		var option:Option = new Option('Hide Winning Icons',
			"If checked, will use the old icons and disable the winning icons\nthis may also fix broken icons for mods that don't use winning icons\nMake sure to put your Icons on the 'images/icons/old' folder",
			'oldIcons', 'bool', false);
		addOption(option);
		...
		var option:Option = new Option('Simply Judgements',
			"If checked, simplifies the judgement animations, displaying only one judgement / rating sprite at a time.", 'simplyJudgements', 'bool', false);
		addOption(option);
		...
		var option:Option = new Option('Lane Opacity', 'Enables a black background behind the notes for visibility\nhow much opaque should it be?',
			'laneAlpha', 'percent', 0);
		option.scrollSpeed = 1.6;
		option.minValue = 0.0;
		option.maxValue = 1;
