...
class Song
{
	...
	public static function loadFromJson(jsonInput:String, ?folder:String):SwagSong
	{
		...
		var formattedFolder:String = Paths.formatToSongPath(folder.startsWith(CoolUtil.invisibleSongPrefix) ? folder.substring(CoolUtil.invisibleSongPrefix.length,
			folder.length) : folder);
		var formattedSong:String = Paths.formatToSongPath(jsonInput.startsWith(CoolUtil.invisibleSongPrefix) ? jsonInput.substring(CoolUtil.invisibleSongPrefix.length,
			jsonInput.length) : jsonInput);
		#if MODS_ALLOWED
